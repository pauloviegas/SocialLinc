...
class instituicaoFinanciadora extends SocialController
{
    ...
    public function index()
    {
        ...
        $this->data['instituicoes'] = $this->grupoModel->recuperaPorParametro(NULL, Array('id_tipo' => 4, 'excluido' => 0));
        ...
        $this->data['permissaoExcluir'] = $this->viewPerfilAcaoModel->verificaPermissao('social/instituicaoFinanciadora/excluir');
    ...
    public function criar()
    {
        ...
        //Redirecionamento
        $this->load->view('instituicaoFinanciadora/criar', $this->data);
    }
    ...
    public function inserir()
    {
        ...
        $this->form_validation->set_rules('nome', 'Nome da Instituição Financiadoraa', 'required|is_unique[sitelinc_gru_grupo.nome]');
        $this->form_validation->set_rules('sigla', 'Sigla da Instituição Financiadoraa', 'required|is_unique[sitelinc_gru_grupo.sigla]');
        ...
        if ($this->form_validation->run())
        {
            ...
            if ($this->grupoModel->inserir($instituicao))
            {
                ...
                redirect('social/instituicaoFinanciadora/index');
            }
            else
            {
                $this->session->set_flashdata(
                        'erro', 'Ops... Ocorreu um erro e a instituição Financiadora'
                        . $instituicao['nome'] . 'não foi inserida com sucesso!');
                redirect('social/instituicaoFinanciadora/criar');
            }
        }
        else
        {
        ...
        if ($this->form_validation->run())
        {
            ...
            else
            {
                $this->session->set_flashdata(
                        'erro', 'Ops... Ocorreu um erro e a instituição Financiadora'
                        . $instituicao['nome'] . ' não foi inserida com sucesso!');
                redirect('social/instituicaoFinanciadora/editar/' . $instituicao['id']);
            }
        }
        else
        {
    ...
    public function excluir()
    {
        ...
        {
            $this->session->set_flashdata(
                    'sucesso', 'A Instituição Financiadora '
                    . $novaInstituicao[0]->nome . ' foi excluida com sucesso com'
                    . ' sucesso!');
            redirect('social/instituicaoFinanciadora/index');
        }
        else
        {
            $this->session->set_flashdata(
                    'erro', 'Ops... Ocorreu um problema e a instituição Financiadora '
