<?php
  ...
  ***************************************/
  ...
  if (!empty($say))
  {
    ...
    $convoArr = run_post_response_useraddons($convoArr); //Roda o BANNED e o Replacer
  ...
  //LOG DE PADROES
  if (isset($convoArr['aiml']['pattern'])) {
	$entrada=(isset($convoArr['aiml']['user_raw'])) ? $convoArr['aiml']['user_raw'] : '';
	$padrao=(isset($convoArr['aiml']['pattern'])) ? $convoArr['aiml']['pattern'] : '';
	$conv=(isset($convoArr['conversation']['convo_id'])) ? $convoArr['conversation']['convo_id'] : '';
	$sql1="INSERT INTO `$dbn`.`conversations`(`id`,`entrada`,`padrao`,`convo_id`,`timestamp`)VALUES(NULL,'$entrada','$padrao','$conv',CURRENT_TIMESTAMP);";
	$sth = $dbConn->prepare($sql1);
    $sth->execute();
  } //fim do log de padrões
